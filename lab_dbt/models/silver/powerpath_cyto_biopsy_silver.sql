...
with raw as (
    select *
    from {{ ref('powerpath_cyto_biopsy_bronze') }}
...
with_tat as (
    ...
    from with_setting w
    left join {{ ref('date_filter_bronze') }} rc
        on cast(w.Received_Date   as date) = rc.calendar_date
    left join {{ ref('date_filter_bronze') }} so
...
historical as (
    ...
    from {{ source('lab', 'powerpath_cyto_biopsy_historical') }}
)
