        ...
        create or replace table `opsanalytics_adb_workspace01`.`lab_staging`.`powerpath_cyto_biopsy_silverv2`
    ...
    using delta
      ...
      with raw as (
    select *
    from `opsanalytics_adb_workspace01`.`lab`.`powerpath_raw`
...
with_tat as (
    ...
    from with_setting w
    left join `opsanalytics_adb_workspace01`.`lab_staging`.`date_spine` rc
        on cast(w.Received_Date   as date) = rc.calendar_date
    left join `opsanalytics_adb_workspace01`.`lab_staging`.`date_spine` so
